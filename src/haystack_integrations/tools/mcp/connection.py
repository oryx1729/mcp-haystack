...
class MCPServer:
    """
    Represents a connection to an MCP server and provides access to all available tools.

    This class manages the connection to an MCP server and allows accessing all tools
    provided by that server. It's an alternative to creating individual MCPTool instances
    when you need access to multiple tools from the same server.

    Example using SSE:
    ```python
    from haystack_integrations.tools.mcp import MCPServer, SSEServerInfo

    # Create server instance
    server = MCPServer(SSEServerInfo(base_url="http://localhost:8000"))
    ...
    Example using stdio:
    ```python
    from haystack_integrations.tools.mcp import MCPServer, StdioServerInfo

    # Create server instance
    server = MCPServer(StdioServerInfo(command="python", args=["path/to/server.py"]))
    ...
    def __init__(
        ...
        :raises MCPConnectionError: If connection to the server fails
        :raises MCPToolNotFoundError: If no tools are available
        ...
        # Connect and initialize tools
        self._connect_and_initialize()

    def _connect_and_initialize(self):
        """
        Connect to the server and initialize tool instances.
        ...
        try:
            ...
            tools = self._run_sync(client.connect(), timeout=self._connection_timeout)
            
            # Handle no tools case
            if not tools:
                message = "No tools available on server"
                raise MCPToolNotFoundError(message=message, tool_name="", available_tools=[])
            ...
            raise MCPConnectionError(message=message, server_info=self._server_info, operation="initialize") from e

    def _create_tool_invoker(self, tool_name: str):
        """
        Create a function that invokes a specific tool.
        
        :param tool_name: Name of the tool to invoke
        :returns: Function that invokes the tool
        """
        def invoke_tool(**kwargs):
            ...
            try:
                ...
                if result.content and all(isinstance(item, types.TextContent) for item in result.content):
                ...
                message = f"Failed to invoke tool '{tool_name}'"
                raise MCPInvocationError(message=message, tool_name=tool_name, tool_args=kwargs) from e
    ...
    def available_tools(self) -> list[Tool]:
        """
        Get list of all available tools on the server.
    ...
    def get_tool(self, name: str) -> Tool:
        ...
        :raises MCPToolNotFoundError: If the tool is not found
        """
        try:
            return self._available_tools[name]
        except KeyError:
            available_tools = list(self._available_tools.keys())
            message = f"Tool '{name}' not found. Available tools: {', '.join(available_tools)}"
    ...
    def close(self) -> None:
        """
        Synchronous version of close that handles the async close internally.
        
        This method ensures all resources are properly released, even if errors occur.
        """
        if self._client:
            try:
                # Use the _run_sync method to run the async close
                self._run_sync(self.aclose())
                
                # Additional cleanup to ensure all resources are released
                if hasattr(self._client, '_process') and self._client._process:
                    try:
    ...
    def __del__(self):
        ...
        if self._client:
            try:
                ...
                logger.warning(f"Error during MCPServer cleanup in __del__: {e}")
